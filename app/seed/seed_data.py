...
and removed later:
  * Question.pyq_paper_id points at a PYQPaper whose label == SEED_SOURCE
...
def clear_seed_data(db) -> None:
    ...
    seed_paper_ids = [p.id for p in db.execute(select(PYQPaper).where(PYQPaper.label == SEED_SOURCE)).scalars().all()]
    if seed_paper_ids:
...
def seed_questions(db, taxonomy: dict, admin_id: uuid.UUID) -> dict:
    ...
    # One PYQPaper per (course, year-or-None) combo, created on demand and
    ...
    papers_cache: dict[tuple[str, int | None], PYQPaper] = {}

    def get_pyq_paper(exam_name: str, year: int | None) -> PYQPaper:
        key = (exam_name, year)
        if key not in papers_cache:
            paper = PYQPaper(exam_name=exam_name, year=year, label=SEED_SOURCE)
            db.add(paper)
            db.flush()
            ...
            for (s_name, t_name), topic in data["topics"].items():
                ...
                for i, (text, options, correct_letter, explanation, difficulty) in enumerate(entries):
                    ...
                    question = Question(
                        ...
                        pyq_paper=get_pyq_paper(course.name, year),
