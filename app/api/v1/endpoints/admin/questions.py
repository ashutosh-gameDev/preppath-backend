...
def list_questions(
    ...
    no_pyq_paper: bool = Query(False, description="Only questions with no PYQ paper tag - for finding ones to clean up"),
    ...
    if pyq_paper_id:
        q = q.where(Question.pyq_paper_id == pyq_paper_id)
    if no_pyq_paper:
        q = q.where(Question.pyq_paper_id.is_(None))
    if language:
